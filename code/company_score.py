...
X_train, X_test, y_train, y_test = train_test_split(df["text"], df["y"], test_size = 0.2, random_state=2023)
                                                   

tfidf_vect = TfidfVectorizer(min_df=2, tokenizer=text_cleaning)
tfidf_vect.fit(X_train)
tfidf_X_train = tfidf_vect.transform(X_train)
tfidf_X_test = tfidf_vect.transform(X_test)
tfidf_vect

invert_index_vectorizer = {v: k for k, v in tfidf_vect.vocabulary_.items()}

lr_clf = LogisticRegression()
lr_clf.fit(tfidf_X_train, y_train)
y_pred = lr_clf.predict(tfidf_X_test)
...
def semantic_analysis(text):
    ...
    score = lr_clf.predict(tfidf_data)
...
## Save pickle
# with open("ml_picklk","wb") as f:
#     pickle.dump(final_df, f)


final_df.to_csv('data/ml.csv', index=False, encoding='utf-8-sig')

# final_df.to_csv('final.csv', sep=',', index=False, encoding='utf-8')